...
const ChatbotScreen = () => {
    ...
    const fetchRecipes = async (query: string) => {
        try {
            ...
            setRecipes(response.data.results);  // storing the response in the state
            await AsyncStorage.setItem('searchedRecipes', JSON.stringify(response.data.results)); // storing in async storage
        } catch (error) {
    ...
    const saveToShoppingList = async (recipe: Recipe) => {
        try {
            ...
            await AsyncStorage.setItem('shoppingList', JSON.stringify(updatedList)); // storing the updated list in async storage
...
const styles = StyleSheet.create({
    ...
    textInputStyle: { marginTop: 40, borderRadius: 8, padding: 10, backgroundColor: '#ffffff', color: '#000000' },
